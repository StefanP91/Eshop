...
const Automotive = () => {
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
            try {
                const response = await fetch(`https://api-eshop-42ioj8rk8-stefan-panovs-projects.vercel.app/automotive_page.php`); 
    ...
    return (
        <div>
            <h1 className='text-center mt-2 text-danger fw-bolder'>Automotive</h1>
                    ...
                    {products.map(product => (
                        <Col key={product.id} lg={3}>
                            <Card className='product-card' onClick={() => navigate(`/productPage/${product.product_id}`)}>
                                ...
                                <Card.Body>
                                    <Card.Title className='fw-bolder text-center'>{product.title}</Card.Title>
